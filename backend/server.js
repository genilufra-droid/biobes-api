...
app.get('/api/state', needDb, needAuth, async (req, res) => {
  try {
    ...
    if (!rows.length) return res.json({ ok: true, state: null, version: 0, updatedAt: null });
    res.json({ ok: true, state: rows[0].data, version: rows[0].version, updatedAt: rows[0].updated_at });
...
app.put('/api/state', needDb, needAuth, async (req, res) => {
  try {
    ...
    const ver = cur.rows.length ? cur.rows[0].version : 0;
    if (baseVersion !== undefined && baseVersion !== null && +baseVersion !== ver) {
      return res.status(409).json({ ok: false, error: 'Konflikt versionesh — ringarko state-in', version: ver });
    }
    await p.query(
      `INSERT INTO app_state(id,data,version,updated_at) VALUES('main',$1::jsonb,$2,NOW())
       ON CONFLICT(id) DO UPDATE SET data=EXCLUDED.data,version=EXCLUDED.version,updated_at=NOW()`,
      [raw, ver + 1]
    );
    await audit(req.user.username, 'STATE_PUT', 'version ' + (ver + 1));
    res.json({ ok: true, version: ver + 1, updatedAt: new Date().toISOString() });
